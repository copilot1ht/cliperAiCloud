...
function New-SignedHeaders(
  ...
  $contentHash = [Convert]::ToHexString($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($Body))).ToLowerInvariant()
...
$workerResult = & python "$PSScriptRoot/qa-cloud-worker.py"
Remove-Item Env:QA_CLOUD_API_BASE, Env:QA_CLOUD_ACCESS_TOKEN, Env:QA_CLOUD_SIGNING_SECRET -ErrorAction SilentlyContinue
$replayBlocked = $false
try {
  Invoke-RestMethod -Method Post -Uri "$ApiBase/v1/chat/completions" -Headers $chatHeaders -Body $chatBody | Out-Null
} catch {
...
$result = [ordered]@{
  ...
  chatContent = $chat.choices[0].message.content
  workerContent = $workerResult
  responseSigned = [bool]$chat.integrity.signature
...
if (-not $result.registered -or -not $result.keyFormat -or $result.activated -ne "active" -or
    -not $result.refreshRotated -or $result.heartbeat -ne "active" -or
    $result.chatContent -ne "MOCK_OK" -or $result.workerContent -ne "MOCK_OK" -or
    -not $result.responseSigned -or -not $result.replayBlocked -or $result.memberRequests -lt 1) {
