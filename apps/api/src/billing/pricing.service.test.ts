...
describe("PricingService", () => {
  it("prices provider cost using the current markup policy", () => {
    ...
    store.updatePricingPolicy({ markupBps: 5000, computeCostMicroUsd: 0, paymentFeeBps: 0, reserveBps: 0 });
    ...
    expect(quote.creditChargeMicro).toBe(200n);
  });

  it("keeps provider cost private from policy overhead calculations", () => {
    const store = new AdminStoreService();
    store.updatePricingPolicy({ markupBps: 2500, computeCostMicroUsd: 20, reserveBps: 500, minimumChargeMicroUsd: 0 });
