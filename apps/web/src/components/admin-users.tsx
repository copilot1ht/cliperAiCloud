...
function UserForm({ user, onClose, onSaved }: { user?: AdminUser; onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      walletUsd: Number(form.get("walletUsd") || 0),
      unlimitedWallet: form.get("unlimitedWallet") === "on",
  ...
  return <AdminModal title={user ? "Edit user" : "Add user"} detail={user ? "Ubah akses, saldo wallet USD, dan batas device." : "Buat akun member lokal dengan password sementara."} onClose={onClose}>
    <form className="admin-form" onSubmit={submit}>
      <div className="form-grid">
        ...
        <label className="check-row"><input name="unlimitedWallet" type="checkbox" defaultChecked={user?.unlimitedWallet || false} /> Saldo unlimited untuk akun internal</label>
      </div>
