...
interface HealthPayload {
  ...
  midtrans: { mode: string; configuration: string; webhookUrl: string; apiReachability: string; lastWebhookAt: string | null; lastSuccessfulPaymentAt: string | null; failedWebhookCount: number; signatureVerification: string };
  warnings: string[];
  errors: string[];
}
...
export function AdminSystemHealth() {
  ...
  return <>
    ...
    <section className="panel"><div className="panel-head"><div><p className="section-kicker">Payment gateway</p><h2>Midtrans</h2><p>Informasi aman tanpa menampilkan Merchant ID, Client Key, atau Server Key.</p></div><span className={statusClass(data.midtrans.configuration === "Ready" ? "healthy" : "not-configured")}>{data.midtrans.mode}</span></div><div className="finance-grid compact-finance"><span><small>Configuration</small><strong>{data.midtrans.configuration}</strong></span><span><small>Webhook</small><strong>{data.midtrans.lastWebhookAt ? formatDate(data.midtrans.lastWebhookAt) : "Belum ada"}</strong></span><span><small>Last paid</small><strong>{data.midtrans.lastSuccessfulPaymentAt ? formatDate(data.midtrans.lastSuccessfulPaymentAt) : "Belum ada"}</strong></span><span><small>Rejected webhooks</small><strong>{data.midtrans.failedWebhookCount}</strong></span></div><div className="callout info-callout"><span><strong>Notification URL:</strong> {data.midtrans.webhookUrl} · Signature: {data.midtrans.signatureVerification} · API: {data.midtrans.apiReachability}</span></div></section>
