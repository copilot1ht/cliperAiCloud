...
interface HealthPayload {
  ...
  payment: { provider: string; label: string; mode: string; configuration: string; source: string; webhookUrl: string; finishRedirectUrl: string; apiReachability: string; lastWebhookAt: string | null; lastSuccessfulPaymentAt: string | null; failedWebhookCount: number; signatureVerification: string };
  warnings: string[];
  errors: string[];
}
...
export function AdminSystemHealth() {
  ...
  return <>
    ...
    <section className="panel"><div className="panel-head"><div><p className="section-kicker">Payment gateway</p><h2>{data.payment.label}</h2><p>Informasi aman tanpa menampilkan credential atau webhook token.</p></div><span className={statusClass(data.payment.configuration === "Ready" ? "healthy" : "not-configured")}>{data.payment.mode}</span></div><div className="finance-grid compact-finance"><span><small>Configuration</small><strong>{data.payment.configuration}</strong></span><span><small>Config source</small><strong>{data.payment.source}</strong></span><span><small>Last paid</small><strong>{data.payment.lastSuccessfulPaymentAt ? formatDate(data.payment.lastSuccessfulPaymentAt) : "Belum ada"}</strong></span><span><small>Rejected webhooks</small><strong>{data.payment.failedWebhookCount}</strong></span></div><div className="callout info-callout"><span><strong>Notification URL:</strong> {data.payment.webhookUrl} · Signature: {data.payment.signatureVerification} · API: {data.payment.apiReachability}</span></div></section>
