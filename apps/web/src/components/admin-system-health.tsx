...
export function AdminSystemHealth() {
  ...
  return <>
    ...
    <section className="panel"><div className="panel-head"><div><p className="section-kicker">Payment gateway</p><h2>{data.payment.label}</h2><p>Configuration, webhook, dan transaksi terakhir dipantau tanpa menampilkan credential.</p></div><span className={statusClass(data.payment.operationalState === "healthy" ? "healthy" : data.payment.operationalState === "failed" ? "offline" : "not-configured")}>{data.payment.operationalState === "healthy" ? "Healthy" : data.payment.operationalState === "failed" ? "Degraded" : "Not tested"}</span></div><div className="finance-grid compact-finance"><span><small>Environment</small><strong>{data.payment.mode}</strong></span><span><small>Configuration</small><strong>{data.payment.configuration}</strong></span><span><small>Last successful payment</small><strong>{data.payment.lastSuccessfulPaymentAt ? formatDate(data.payment.lastSuccessfulPaymentAt) : "Belum ada"}</strong></span><span><small>Last verified webhook</small><strong>{data.payment.lastWebhookAt ? formatDate(data.payment.lastWebhookAt) : "Belum ada"}</strong></span></div><div className="callout info-callout"><span><strong>Notification URL:</strong> {data.payment.webhookUrl} · Signature: {data.payment.signatureVerification} · API: {data.payment.apiReachability}</span></div></section>
