...
import { adminFetch, formatIdr, formatUsd, type PricingPolicy } from "@/lib/admin-api";
...
function PricingPolicyEditor({ policy, onSaved }: { policy: PricingPolicy; onSaved: () => void }) {
  ...
  const impliedMargin = markupPercent > 0 ? markupPercent / (100 + markupPercent) * 100 : 0;
  return <section className="panel pricing-panel"><div className="panel-head"><div><p className="section-kicker">Cost-based pricing</p><h2>Pricing policy</h2><p>Admin mengatur target markup atas service cost. Gross margin dihitung otomatis dan tidak dapat diisi manual.</p></div><Settings2 size={19} /></div>
    <form className="pricing-form" key={policy.updatedAt} onSubmit={submit}>
      ...
      <div className="pricing-preview"><small>Example before overhead</small><span><b>Cost 100</b><i>+</i><b>{markupPercent}% markup</b><i>=</i><strong>{(100 * (1 + markupPercent / 100)).toFixed(2)} user price</strong></span><small>Implied gross margin: {impliedMargin.toFixed(2)}%</small></div>
...
export function AdminRevenue() {
  ...
  const load = useCallback(() => { setError(""); adminFetch<RevenuePayload>("/api/admin/revenue").then(setData).catch((reason) => setError(reason.message)); }, []);
  useEffect(load, [load]);
  ...
  return <>
    ...
    <div className="two-column">
      <section className="panel"><div className="panel-head"><div><p className="section-kicker">Revenue definition</p><h2>What this page measures</h2><p>Revenue adalah uang masuk dari subscription/top-up user. Provider cost adalah biaya aktual AI. Gross margin AI adalah billed value dikurangi provider cost.</p></div></div><div className="economics-equation"><span><small>User billing</small><strong>{formatUsd(data.usage.billedCostUsd)}</strong></span><b>−</b><span><small>Provider cost</small><strong>{formatUsd(data.usage.providerCostUsd)}</strong></span><b>=</b><span className="result"><small>Gross margin</small><strong>{formatUsd(data.usage.grossMarginUsd)}</strong></span></div></section>
