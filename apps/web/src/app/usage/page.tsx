...
function credits(value: number): string {
  return new Intl.NumberFormat("id-ID", { maximumFractionDigits: 3 }).format(Number(value || 0) / 1_000_000);
}
...
export default function UsagePage() {
  ...
  const exportCsv = () => {
    ...
    const rows = [["module", "tokens", "latency_ms", "credits", "created_at"], ...data.usage.recent.map((item) => [item.module, item.tokens, item.latencyMs, credits(item.creditChargeMicro), item.createdAt])];
  ...
  return <AppShell eyebrow="Consumption" title="Usage & credits" actions={<button className="button button-secondary" onClick={exportCsv} disabled={!data?.usage.recent.length}><Download size={16} /> Export CSV</button>}>
    ...
    {data && <>
      <section className="stats-grid">
        ...
        <StatCard label="Credits spent" value={credits(data.usage.creditChargeMicro)} detail={`${credits(data.credits.availableMicro)} available`} icon={Coins} tone="coral" />
      </section>
      <section className="panel table-panel"><div className="panel-head"><div><p className="section-kicker">Immutable usage view</p><h2>Recent activity</h2><p>Nilai akan persisten setelah PostgreSQL repository diaktifkan.</p></div><button className="button button-secondary" onClick={() => void load()}>Refresh</button></div>{data.usage.recent.length ? <div className="table-scroll"><table><thead><tr><th>Module</th><th>Tokens</th><th>Latency</th><th>Credits</th><th>Time</th></tr></thead><tbody>{data.usage.recent.map((item) => <tr key={item.id}><td><strong>{item.module}</strong></td><td>{item.tokens.toLocaleString("id-ID")}</td><td>{item.latencyMs} ms</td><td>{credits(item.creditChargeMicro)}</td><td>{formatDate(item.createdAt)}</td></tr>)}</tbody></table></div> : <div className="admin-empty"><strong>Belum ada request</strong><span>Aktivitas akan muncul setelah Cliper Studio menggunakan AI Gateway.</span></div>}</section>
    </>}
  </AppShell>;
}
