...
function readConfig() {
  try {
    ...
    if (!fs.existsSync(configPath)) {
      return { providerType: "cloud", baseUrl: defaultCloudBaseUrl(), cloudBaseUrl: defaultCloudBaseUrl(), model: "auto", highlightModel: "auto" };
    }
...
async function verifyCliperCloud(payload = {}) {
  ...
  try {
    ...
    return {
      ...
      walletReady,
      status: unlimited
        ? `Cloud terhubung · ${license.plan || "plan"} · saldo ${creditLabel}`
        : spendableUsd > 0
          ? `Cloud terhubung · ${license.plan || "plan"} · saldo ${creditLabel}`
          : `Cloud terhubung · ${license.plan || "plan"} · isi saldo sebelum pekerjaan AI berbayar`,
    ...
    if (providerUnavailable) {
      return {
        ok: true,
        routerReady: false,
        status: `Cloud terhubung · ${license.plan || "plan"} · AI provider belum disiapkan admin`,
